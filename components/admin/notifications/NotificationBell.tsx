...
export default function NotificationBell() {
  const [count,setCount]=useState(0); const [config,setConfig]=useState<Config|null>(null); const [busy,setBusy]=useState(false); const [status,setStatus]=useState(""); const [canEnableAlerts,setCanEnableAlerts]=useState(false);
  ...
  useEffect(()=>{
    // Keep the server and first client render identical, then read the browser-only permission.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setCanEnableAlerts("Notification" in window && window.Notification.permission!=="granted");
  ...
  async function enable(){setBusy(true);setStatus("");try{
    ...
    const response=await fetch("/api/admin/notifications",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"register-device",token,deviceName:/Android/i.test(navigator.userAgent)?"Chrome on Android":"Chrome browser",browser:navigator.userAgent.slice(0,80)})});const result=await response.json() as {success?:boolean;message?:string};if(!response.ok||!result.success)throw new Error(result.message||"Device registration failed.");setCanEnableAlerts(false);setStatus("Notifications enabled");
  }catch(error){setStatus(error instanceof Error?error.message:"Notifications could not be enabled.");}finally{setBusy(false)}}
  return <div className="relative flex items-center gap-1.5"><Link href="/admin/notifications" aria-label={`${count} unread notifications`} className="relative flex h-10 w-10 items-center justify-center rounded-2xl border border-[#E5DBEA] bg-white text-[#632B87] hover:bg-[#F7F2FA]">{count?<BellRing size={19}/>:<Bell size={19}/>} {count?<span className="absolute -right-1 -top-1 min-w-5 rounded-full bg-red-600 px-1 text-center text-[10px] font-black leading-5 text-white">{count>99?"99+":count}</span>:null}</Link>{canEnableAlerts?<button type="button" disabled={busy} onClick={()=>void enable()} className="hidden min-h-10 rounded-xl bg-[#F2EAF6] px-3 text-[11px] font-black text-[#632B87] xl:inline-flex xl:items-center xl:gap-1.5">{busy?<Loader2 className="animate-spin" size={14}/>:<Bell size={14}/>} Enable alerts</button>:null}{status?<span className="sr-only" role="status">{status}</span>:null}</div>;
}
